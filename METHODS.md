...
## All-model overview and H100 shading

The landing page shows every registered candidate/cohort record across model scales. The numeric x coordinate uses a verified whole-model compression factor. Local or ambiguous sizes and missing quality have separate inventory columns. A selected point exposes its benchmarks and original matched comparison. No cross-model quality score or pooled Pareto front is constructed.

H100 background bands are **weight-storage estimates, not verified runtime requirements**. The default assumes 80 decimal GB per card and reserves 10 GB per card; both fields are adjustable. For registered whole-model bytes W and usable bytes C, the displayed count is ceil(W/C), assuming ideal sharding. Peak resident memory, KV cache, activation and workspace costs are not measured by this estimate. Local-cell and ambiguous-checkpoint panels have no shading. Model-scale rows use their exact original byte denominator only when unambiguous; no byte denominator is inferred from the nominal B label. Colors, patterns and text labels remain distinguishable in monochrome and SVG exports.

The new 1B, 7B and 72B rollout starts with local native FFN and attention-Q fits at 25%/50% active-value budgets. These are not measured whole-model compression factors. Complete compositions, matched quantization/deletion controls and per-scale capability evaluations follow qualification. The 128B tier awaits an exact checkpoint identity; historical 120B MoE work is not renamed.
