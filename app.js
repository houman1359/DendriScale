...
function shapeMark(key,x,y,size=5.5){
 ...
 else if(style.shape==='star'){tag='polygon';attrs={points:Array.from({length:10},(_,i)=>{const r=i%2?size*.55:size*1.35,t=-Math.PI/2+i*Math.PI/5;return `${x+r*Math.cos(t)},${y+r*Math.sin(t)}`;}).join(' ')};}
...
function selectPanels(){plotZoom.reset();let ps=DATA.panels.filter(p=>UI.canonicalModelId(p.model)===byId('model').value&&p.source_level===byId('level').value);
 ...
 opts(byId('panel'),ps.map(p=>[p.id,UI.metricName(p.ylabel)+' · '+UI.axisName(p)+(['registry-reported','external-reported'].includes(p.source_level)?' · '+p.cohort:'')]));selectDose();}
...
function detail(v,p){
 ...
 UI.result(byId('detail'),{title:v.label,subtitle:UI.modelName(v.model)+' · '+methodLabel(methodKey(v)),state:v.label==='Teacher reference'?'reference':outcome(v),status:v.gate_status,allPass:UI.allBenchmarksPass(v),
  stats:[[UI.metricName(p.ylabel),num(v.y)+(denominator?' / '+num(denominator):'')],[accounting.whole?(external?'Reported model size':'Whole-model registered size'):UI.axisName(p),accounting.bytes?(accounting.bytes/1e9).toFixed(4)+' GB':accounting.whole?'Byte size not linked':cost(p,v.x)],[external?'Reported compression (bytes)':'Whole-model compression (bytes)',accounting.factor?accounting.factor.toFixed(3)+'×':accounting.whole?'Reference not linked':'Not measured']].slice(0,fileOnly?2:3),
...
function draw(){const p=current(),svg=byId('plot');svg.replaceChildren();byId('points').replaceChildren();UI.empty(byId('detail'));delete byId('detail').dataset.pointId;if(!p)return;
 ...
 byId('protocolWarning').hidden=!(UI.historicalProtocol(p)&&p.source_level==='curated');byId('externalWarning').hidden=p.source_level!=='external-reported';
 ...
 byId('axisExplanation').textContent=p.source_level==='external-reported'?(p.xunit==='percent'?'X = reported size as a percentage of the reported FP16 original from the same source. Reported by PrismML; not a DendriScale byte ledger.':'X = model size as reported by PrismML, in decimal GB; Conditions give the exact basis. Not a DendriScale byte ledger.'):p.xunit==='percent'?'X = whole-model bytes retained. 25% retained means 4× smaller. Quantization and connectivity costs are included.':p.points?.[0]?.xaxis==='weight_file_bytes'?'X = delivered weight-file size, including stored auxiliary tensors. This is neither GPU memory nor complete executed text-model accounting. See Conditions and Limitations.':p.points?.[0]?.xaxis==='whole_registered_bytes'?'X = complete-model registered size, including stored weights, connectivity, scales and buffers. GPU working memory is measured separately.':p.points?.[0]?.xaxis==='whole_compression'?'X = original reference model bytes ÷ compressed model bytes. This is byte compression, including quantization.':'X = the recorded local resource below. Whole-model byte compression is reported separately when available.';
